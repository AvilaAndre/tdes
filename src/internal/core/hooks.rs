...
pub struct SimulationHooks {
    ...
    pub on_simulation_finish_hook: CustomOnFinishHook,
    /// If it is set and returns true, then
    /// the simulation loop finishes.
...
impl Default for SimulationHooks {
    fn default() -> Self {
        Self {
            on_simulation_finish_hook: Box::new(|_ctx| {}),
...
impl SimulationHooks {
    pub fn set_on_simulation_finish_hook(&mut self, hook: CustomOnFinishHook) -> &mut Self {
        self.on_simulation_finish_hook = hook;
        self
    }
